...
23. 对于一个follower可以对不同term进行一票多投问题，然而确实在选举过程中，
可能同一时间产生多个leader，然后只能发送和接收心跳时小term才能感知到心跳才能变成follower，然而
这一时间确实有两个leader存在，若是worker判断自己是否主然后执行任务，那么可能就产生多个正在
执行的worker，如何修改？强制本term收到一个心跳才行？而我们这里就可以设定一个全局的变量，不对，
这个是进程间通信，若不借助第三方公共内存，那么如何实现？感觉只有查看自己是否收到新term的心跳才可以呢？
或者存在每个节点状态？那一样会有当时我是leader而未收到两一个leader时候。

--ok
目前想到的就是强制等待本term的第一个心跳或者发送了第一个心跳后，即能完全确定大家同意了，存在统一
的leader。
这种情况存在于，异常情况时，不同term进行选举产生，而还未统一之前的临时状态。孤岛恢复也出不来这个问题。
那这种情况很极端啊，一个node肯定是增长了比别人快的term，然后得到了新的term的leader，term++只能
发生在超时投票，那么就是超时时间很短？还是有啥异常原因。。

24. 对于同一个node，可能存在收到投票了，然后同时自己也发送投票场景？
需要保证变量可见性+同步处理投票、接收投票(/心跳)问题。
想了下还是不要用同步处理，因为虽然并发小，但是也最好要最快反应，那么就是谁先来先处理谁，使用cas进行操作。
若与此同时有其他人操作也可以，不过最后修改状态的只有一个(也就是早的那个)能成功。
想了下，应该有状态优先级吧。
不用优先级，只需要思考一下状态的转换以及前后置即可。
对于发起投票，前置是长时间没有收到投票或心跳，低的term直接决绝，所以当醒来时可以进行状态临时变更，当要发起投票
时，查看本地节点若没变化，那么就发起投票了，若是有变化，那必然有高term进行投票或者心跳了，而自己必然同意了，
那么自己就再等一次再发起投票吧，人家优先了。
对于接收投票、心跳，只要term高，那么就同意，则更新本地node状态。
对于处理投票回应，若本地node变更，则表明有高term来到了，自己统计的肯定也不会是多数，另外若有高term的心跳，
那么同样我也同意，也就不用统计了。若都没变动，则我成为leader，发起心跳。